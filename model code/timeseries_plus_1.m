...
for num = 1:739
    ...
    if num2>=10
        ...
        [para,~] = fmincon(@(para) likelihood_plus_1(test_cascade_list,para),[rand(1)*10;rand(1)*1],[],[],[],[],[0.01;0.01],[0.1;1],[],options);
          for i=1:interval
              test_index2 = test_cascade_list(find(test_cascade_list<T/interval*i)) ;
              output(num,i) = length(test_index2);
              output(num,i) = output(num,i)+exp((para(1)*T*2-length(test_index2)*para(2))/20)/para(1)-exp((para(1)*T-length(test_index2)*para(2))/20)/para(1);
          end
    end
end
